...
def ejecutar_revision():
  ...
  try:
    ...
    print(f"Vela cerrada {hora_vela.strftime('%H:%M')} | Precio: {precio_actual:.2f} | RSI: {rsi_val:.2f}")

    # Evaluación de señales y envío
    if ha["En_Horario"].iloc[idx]:
      ...
      print(f"⏳ Fuera de horario de operación (Vela de las {hora_vela.strftime('%H:%M')}). No se enviarán señales por Telegram.")
